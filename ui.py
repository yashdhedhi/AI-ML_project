...
def _inject_theme_css():
    """Inject global CSS for custom theming (dark only)."""
    ...
    # 🎯 GLOBAL TEXT COLOR (as requested)
    text_main = "#89a2d4"
    text_muted = "#89a2d4"
    ...
    /* ===== PRIMARY BUTTONS (TEXT BLACK – KEEP AS IS) ===== */
    ...
    /* ===== FILE UPLOADER BUTTON (TEXT BLACK) ===== */
    button[data-testid="baseButton-secondary"] {{
        color: #000000 !important;
        font-weight: 600 !important;
    }}

    /* ===== LINK BUTTONS (SKILLS ETC.) ===== */
    ...
    /* ===== GLOBAL TEXT OVERRIDE (IMPORTANT PART) ===== */
    h1, h2, h3, h4, h5, h6,
    p, label, span, li, a,
    div, strong, em {{
...
def render_topbar(active: str = "Home"):
    ...
    with nav_cols[3]:
        st.page_link("pages/5_Learning_Path.py", label="📚 Learning Path")
    with nav_cols[4]:
        st.page_link("pages/7_Interview_Prep.py", label="🎤 Interview Prep")

    with st.sidebar:
        st.markdown('<div class="sidebar-logo">🚀 AI Career Assistant</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="sidebar-tagline">From resume to job, skills, and interviews — all in one place.</div>',
            unsafe_allow_html=True,
        )

        st.markdown('<div class="sidebar-section-title">Main</div>', unsafe_allow_html=True)
        st.page_link("app.py", label="🏠 Home")
        st.page_link("pages/2_Login.py", label="🔐 Login / Sign up")
        st.page_link("pages/3_Saved_Jobs.py", label="💾 Saved Jobs")

        st.markdown('<div class="sidebar-section-title">Growth</div>', unsafe_allow_html=True)
        st.page_link("pages/5_Learning_Path.py", label="📚 Learning Path")
        st.page_link("pages/6_My_Learning_Paths.py", label="🧾 My Learning Paths")

        st.markdown('<div class="sidebar-section-title">Interviews</div>', unsafe_allow_html=True)
        st.page_link("pages/7_Interview_Prep.py", label="🎤 Interview Prep")
